...
class DistilBertCrossAttention(nn.Module):
    def __init__(self, num_labels=2, metadata_size=22):
        ...
        self.cross_attention = CrossAttentionLayer(
            hidden_dim=self.config.dim,  # DistilBERT hidden dim
...
def create_archive_directory(base_dir="./training/distilbert-cross-attention-imdb-archive"):
...
def finalize_archive(model, tokenizer, test_dataset, training_args, eval_results, config, archive_dir, genre_sentiment_stats=None):
    ...
    with open(os.path.join(model_dir, "config.json"), "w") as f:
        json.dump({
            "model_type": "distilbert-cross-attention",
            "num_labels": 2,
...
def main():
    ...
    model = DistilBertCrossAttention(num_labels=2, metadata_size=metadata_size)
    ...
    config = {
        ...
        "description": "IMDB sentiment analysis using cross-attention between text and metadata with genre sentiment embeddings"
    }
