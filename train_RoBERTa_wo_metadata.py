...
MAX_LENGTH = 512  # Changed from 256 to 512 to match metadata script
...
def create_archive_directory(base_dir="./training/roberta-wo-metadata-imdb-archive"):
...
def main():
    ...
    # Training arguments - using archive directory paths with memory optimizations
    training_args = TrainingArguments(
        output_dir=os.path.join(archive_dir, "training_outputs"),
        num_train_epochs=3,
        per_device_train_batch_size=8,  # Keep at 8 for memory
        per_device_eval_batch_size=8,   # Keep at 8 for memory
        ...
        do_eval=True,
        no_cuda=False,
        dataloader_pin_memory=False,    # Keep disabled for memory efficiency
        fp16=True,                      # Keep mixed precision
        dataloader_num_workers=0,       # Keep reduced workers
        remove_unused_columns=True,     # Keep for efficiency
        eval_accumulation_steps=10      # Keep for memory efficiency
    )
    ...
    config = {
        ...
        "epochs": 3,
        "batch_size": 8,              # Keep at 8 to match actual training
        "max_length": MAX_LENGTH,     # Will now be 512
        "experiment_date": datetime.now().strftime("%Y-%m-%d"),
        "experiment_time": datetime.now().strftime("%H:%M:%S"),
        "description": "IMDB sentiment analysis using RoBERTa with text only (without metadata)"
    }
